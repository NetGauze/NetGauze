...
impl ValidationActor {
    ...
    fn create_subscription(
        ...
        let ctx = Context::new_from_yang_library_file(yanglibrary, format, search_dir, flags)
            .expect("Failed to create context");
        debug!("modules: {}", ctx.modules(false).count());
        ...
        &self.subscriptions[&(peer, subscr_id)]
    }
}
...
mod tests {
    ...
    fn create_actor() -> ValidationActor {
        ValidationActor::new(
            ...
            super::ValidationStats::new(opentelemetry::global::meter("test_meter")),
        )
    }
