...
use crate::{
    model::/*{*/notification::SubscriptionId/*, udp_notif::UdpNotifPacketDecoded}*/,
    //schema_cache::YangSchema,
};
...
impl ValidationActor {
            ...
            tokio::select! {
                ...
                */
                msg = self.udp_notif_rx.recv() => {
                    match msg {
                        Ok(arc_tuple) => {
                            let (peer, packet) = Arc::try_unwrap(arc_tuple).unwrap();
                            let peer_tags = [
                                opentelemetry::KeyValue::new(
    ...
    fn handle_udp_notif(
        ...
        let ctx = self.get_context(peer, subscr_id);
        match ctx {
            Ok(ctx) => ctx,
            Err(err) => {
                error!("Failed to get context for peer {}: {}", peer, err);
                return Err(ValidationActorError::YangPushReceiveError);
            }
        };

        // Parse the Yang Push notification content
        let yanglibrary = "E96CB84D-F02B-4FBF-BE86-3580300CD964"; // TODO
        let search_dir = "models"; // TODO
        let ctx = self.create_context(yanglibrary, search_dir)?;
