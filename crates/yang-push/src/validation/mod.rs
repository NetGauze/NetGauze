...
struct CachedSubscription {
    ...
    cached_packets: Vec<Arc<(SocketAddr, SocketAddr, UdpNotifPacket)>>,
}
...
struct ValidationActor {
    ...
    rx: async_channel::Receiver<Arc<(SocketAddr, SocketAddr, UdpNotifPacket)>>,
...
impl ValidationActor {
    ...
    fn cache_packet(
        &mut self,
        subscription_info: SubscriptionInfo,
        message: Arc<(SocketAddr, SocketAddr, UdpNotifPacket)>,
    ) -> bool {
        let (peer, _collector, packet) = message.as_ref();
        let peer = *peer;
    ...
    async fn process_udp_notif_msg(
        ...
        let (peer, _, packet) = message.as_ref();
        let peer = *peer;
    ...
    async fn extract_subscription_info(
        ...
        let (_, collector, packet) = message.as_ref();
        let mut peer_tags = Self::peer_tags_from_packet(peer, packet);
        let message_id = decoded.message_id();
        let publisher_id = decoded.publisher_id();
        let notification_type = decoded
    ...
    async fn process_cache_response(
        ...
        for msg in cached_packets {
            let (peer, _collector, packet) = msg.as_ref();
            let peer = *peer;
            let mut peer_tags = Self::peer_tags_from_packet(peer, packet);
            Self::extend_peer_targs_with_subscription_info(&subscription_info, &mut peer_tags);
