...
//! - **Worker panics**: Logged as join errors; actor continues processing
//!
//! # Shutdown Behavior
//!
...
pub enum CacheLookupCommand {
    LookupBySubscriptionInfo(
        SubscriptionInfo,
        async_channel::Sender<Option<Arc<YangLibraryReference>>>,
    ),

    LookupBySubscriptionInfoOneShot(
        SubscriptionInfo,
        oneshot::Sender<Option<Arc<YangLibraryReference>>>,
    ),
...
impl std::fmt::Display for CacheLookupCommand {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            CacheLookupCommand::LookupBySubscriptionInfo(subscription_info, _) => {
                write!(f, "lookup by subscription info {subscription_info}")
            }
            CacheLookupCommand::LookupBySubscriptionInfoOneShot(subscription_info, _) => {
                write!(
                    f,
                    "lookup by subscription info {subscription_info} (one shot)"
                )
            }
            CacheLookupCommand::LookupByContentId(content_id, _) => {
                write!(f, "lookup by content id {content_id}")
            }
            CacheLookupCommand::LookupByContentIdOneShot(content_id, _) => {
...
struct CacheActor<F: YangLibraryFetcher> {
    ...
    fetcher: F,
    pending_requests:
        FxHashMap<SubscriptionInfo, Vec<async_channel::Sender<Option<Arc<YangLibraryReference>>>>>,
...
impl<F: YangLibraryFetcher> CacheActor<F> {
    async fn send_yang_lib_ref(
        ...
        sender: async_channel::Sender<Option<Arc<YangLibraryReference>>>,
    ) {
        let hit = yang_lib_ref.is_some();
        match sender.send(yang_lib_ref).await {
            Ok(_) => {
    ...
    fn send_yang_lib_ref_oneshot(
        subscription_info: &SubscriptionInfo,
        yang_lib_ref: Option<Arc<YangLibraryReference>>,
        sender: oneshot::Sender<Option<Arc<YangLibraryReference>>>,
    ) {
        let hit = yang_lib_ref.is_some();
        match sender.send(yang_lib_ref) {
            Ok(_) => {
