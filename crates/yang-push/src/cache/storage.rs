...
pub enum YangLibraryCacheError {
    ...
    #[strum(to_string = "xml parsing error while parsing the yang libarary: {0}")]
...
impl YangLibraryCache {
    ...
    pub fn from_disk(cache_root_path: PathBuf) -> Result<Self, YangLibraryCacheError> {
        ...
        for dir in std::fs::read_dir(cache_root_path.as_path())? {
            let path = dir?.path();
            if !path.is_dir() {
                continue;
            }
            ...
            let subscription_info_file = std::fs::File::open(subscription_info_path)?;
