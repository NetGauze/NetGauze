...
pub enum BgpMessageParsingError {
    #[error("while parsing BGP message: {0}")]
    ...
    #[error(
        "connection is not synchronized at byte offset {offset}: the 16-byte message marker must be all ones, got {header:#034x}"
    )]
    ConnectionNotSynchronized { offset: usize, header: u128 },

    /// Couldn't recognize the type octet in the BGPMessage
    #[error("unknown BGP message type {code} at byte offset {offset}")]
    UndefinedBgpMessageType { offset: usize, code: u8 },

    /// BGP Message length is not in the defined \[min, max\] range for the
    /// given message type
    #[error(
        "invalid BGP message length {length} at byte offset {offset} (must be at least {BGP_MIN_MESSAGE_LENGTH}, at most {BGP_MAX_MESSAGE_LENGTH} for OPEN and KEEPALIVE, and must not exceed the bytes available)"
    )]
    BadMessageLength { offset: usize, length: u16 },

    #[error(
        "{unparsed_bytes} trailing byte(s) left unparsed at byte offset {offset} in a BGP message declaring length {length}"
    )]
    UnparseableBytes {
        offset: usize,
        length: u16,
