...
pub enum RouteDistinguisherParsingError {
    ...
    #[error("unknown route distinguisher type {code} at byte offset {offset}")]
    UndefinedRouteDistinguisherTypeCode { offset: usize, code: u16 },

    #[error(
        "malformed leaf-A-D route distinguisher {num1}:{num2} at byte offset {offset} (must be all ones)"
    )]
...
pub enum LabeledNextHopParsingError {
    #[error("while parsing labeled next hop: {0}")]
    Parse(#[from] ParseError),

    #[error(
        "invalid labeled next hop length {len} at byte offset {offset} (expected {LABELED_IPV4_LEN} or {LABELED_IPV6_LEN})"
    )]
...
pub enum Ipv4MplsVpnUnicastAddressParsingError {
    ...
    #[error("invalid prefix length {len} at byte offset {offset}")]
    InvalidLength { offset: usize, len: u8 },

    #[error("in route distinguisher: {0}")]
    RouteDistinguisherError(#[from] RouteDistinguisherParsingError),

    #[error("in unicast prefix: {0}")]
    Ipv4UnicastError(#[from] Ipv4UnicastParsingError),

    #[error("in label stack: {0}")]
...
pub enum Ipv6UnicastParsingError {
    ...
    #[error("{network} is not a valid IPv6 unicast network at byte offset {offset}")]
...
pub enum Ipv6MulticastParsingError {
    ...
    #[error("{network} is not a valid IPv6 multicast network at byte offset {offset}")]
...
pub enum Ipv4UnicastParsingError {
    ...
    #[error("{network} is not a valid IPv4 unicast network at byte offset {offset}")]
...
pub enum Ipv4MulticastParsingError {
    ...
    #[error("{network} is not a valid IPv4 multicast network at byte offset {offset}")]
...
pub enum MacIpAdvertisementParsingError {
    ...
    #[error(
        "invalid MAC address length {length} at byte offset {offset} (expected {MAC_ADDRESS_LEN_BITS})"
    )]
    InvalidMacAddressLength { offset: usize, length: u8 },

    #[error(
        "invalid IP address length {length} at byte offset {offset} (expected 0, {IPV4_LEN_BITS}, or {IPV6_LEN_BITS})"
    )]
...
pub enum InclusiveMulticastEthernetTagRouteParsingError {
    ...
    #[error(
        "invalid IP address length {length} at byte offset {offset} (expected {IPV4_LEN_BITS} or {IPV6_LEN_BITS})"
    )]
    InvalidIpAddressAddressLength { offset: usize, length: u8 },
...
pub enum L2EvpnRouteParsingError {
    ...
    #[error("in ethernet auto-discovery route: {0}")]
    EthernetAutoDiscoveryError(#[from] EthernetAutoDiscoveryParsingError),

    #[error("in MAC/IP advertisement route: {0}")]
    MacIpAdvertisementError(#[from] MacIpAdvertisementParsingError),

    #[error("in inclusive multicast ethernet tag route: {0}")]
...
pub enum L2EvpnIpPrefixRouteParsingError {
    #[error(
        "invalid buffer length {length} at byte offset {offset} (expected {L2_EVPN_IPV4_PREFIX_ROUTE_LEN} or {L2_EVPN_IPV6_PREFIX_ROUTE_LEN})"
    )]
...
pub enum RouteTargetMembershipAddressParsingError {
    ...
    #[error(
        "invalid route target membership prefix length {length} at byte offset {offset} (must be 0 or 32-96)"
    )]
    InvalidPrefixLen { offset: usize, length: u8 },

    #[error("in membership: {0}")]
...
pub enum Ipv4NlriMplsLabelsAddressParsingError {
    ...
    #[error("{error} at byte offset {offset}")]
    InvalidIpv4NlriMplsLabelsAddress {
        offset: usize,
        error: InvalidIpv4NlriMplsLabelsAddress,
    },

    #[error("invalid prefix length {len} at byte offset {offset} (shorter than the label stack)")]
    InvalidLength { offset: usize, len: u8 },
}
