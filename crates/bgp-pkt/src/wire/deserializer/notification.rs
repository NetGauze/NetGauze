...
pub enum BgpNotificationMessageParsingError {
    ...
    #[error("unknown notification code {code} at byte offset {offset}")]
...
impl<'a> ParseFrom<'a> for BgpNotificationMessage {
    ...
    fn parse(cur: &mut SliceReader<'a>) -> Result<Self, Self::Error> {
        ...
        let notification_type = BgpErrorNotificationCode::try_from(code).map_err(|err| {
...
pub enum MessageHeaderErrorParsingError {
    ...
    #[error("unknown message header error subcode {code} at byte offset {offset}")]
...
impl<'a> ParseFrom<'a> for MessageHeaderError {
    ...
    fn parse(cur: &mut SliceReader<'a>) -> Result<Self, Self::Error> {
        let code = cur.peek_u8()?;
        let sub_code = MessageHeaderErrorSubCode::try_from(code).map_err(|err| {
...
pub enum OpenMessageErrorParsingError {
    ...
    #[error("unknown open message error subcode {code} at byte offset {offset}")]
...
impl<'a> ParseFrom<'a> for OpenMessageError {
    ...
    fn parse(cur: &mut SliceReader<'a>) -> Result<Self, Self::Error> {
        let code = cur.peek_u8()?;
        let sub_code = OpenMessageErrorSubCode::try_from(code).map_err(|err| {
...
pub enum UpdateMessageErrorParsingError {
    ...
    #[error("unknown update message error subcode {code} at byte offset {offset}")]
...
impl<'a> ParseFrom<'a> for UpdateMessageError {
    ...
    fn parse(cur: &mut SliceReader<'a>) -> Result<Self, Self::Error> {
        let code = cur.peek_u8()?;
        let sub_code = UpdateMessageErrorSubCode::try_from(code).map_err(|err| {
...
pub enum FiniteStateMachineErrorParsingError {
    ...
    #[error("unknown finite state machine error subcode {code} at byte offset {offset}")]
...
impl<'a> ParseFrom<'a> for FiniteStateMachineError {
    ...
    fn parse(cur: &mut SliceReader<'a>) -> Result<Self, Self::Error> {
        let code = cur.peek_u8()?;
        let sub_code = FiniteStateMachineErrorSubCode::try_from(code).map_err(|err| {
...
pub enum CeaseErrorParsingError {
    ...
    #[error("unknown cease error subcode {code} at byte offset {offset}")]
    Undefined { offset: usize, code: u8 },
}
...
impl<'a> ParseFrom<'a> for CeaseError {
    ...
    fn parse(cur: &mut SliceReader<'a>) -> Result<Self, Self::Error> {
        let code = cur.peek_u8()?;
        let sub_code =
            CeaseErrorSubCode::try_from(code).map_err(|err| CeaseErrorParsingError::Undefined {
                offset: cur.offset(),
                code: err.0,
            })?;
        let _code = cur.read_u8()?;
...
pub enum RouteRefreshErrorParsingError {
    ...
    #[error("unknown route refresh error subcode {code} at byte offset {offset}")]
    Undefined { offset: usize, code: u8 },
}
...
impl<'a> ParseFrom<'a> for RouteRefreshError {
    ...
    fn parse(cur: &mut SliceReader<'a>) -> Result<Self, Self::Error> {
        let code = cur.peek_u8()?;
        let sub_code = RouteRefreshMessageErrorSubCode::try_from(code).map_err(|err| {
