...
pub enum BgpLsAttributeParsingError {
    ...
    #[error("unknown BGP-LS attribute TLV type {error} at byte offset {offset}")]
    UnknownTlvType {
        offset: usize,
        error: BgpLsAttributeTypeError,
    },
    #[error("invalid UTF-8 in BGP-LS attribute at byte offset {offset}: {error}")]
    Utf8Error { offset: usize, error: String },
    #[error(
        "invalid IP address length {length} in BGP-LS attribute at byte offset {offset} (expected 4 for IPv4 or 16 for IPv6)"
    )]
    WrongIpAddrLength { offset: usize, length: usize },
    #[error("in MPLS label: {0}")]
    MplsLabelParsingError(#[from] MplsLabelParsingError),
    #[error("invalid BGP-LS SID value {value} at byte offset {offset}")]
