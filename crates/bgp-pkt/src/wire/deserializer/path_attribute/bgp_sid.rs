...
use crate::wire::deserializer::{count_tlvs_t8_l16, read_tlv_header_t8_l16};
...
pub enum BgpPrefixSidTlvParsingError {
    ...
    #[error("unknown BGP Prefix-SID TLV type {error} at byte offset {offset}")]
    BadBgpPrefixSidTlvType {
        offset: usize,
        error: BgpSidAttributeTypeError,
    },

    #[error("in SRv6 SRGB: {0}")]
...
pub enum BgpPrefixSidSubTlvParsingError {
    #[error("{0}")]
    Parse(#[from] ParseError),
    #[error("unknown BGP Prefix-SID sub-TLV type {error} at byte offset {offset}")]
...
pub enum BgpPrefixSidSubSubTlvParsingError {
    ...
    #[error("unknown BGP Prefix-SID sub-sub-TLV type {error} at byte offset {offset}")]
...
impl<'a> ParseFromWithOneInput<'a, bool> for PrefixSegmentIdentifier {
    ...
    fn parse(cur: &mut SliceReader<'a>, extended_length: bool) -> Result<Self, Self::Error> {
        ...
        let mut tlvs = Vec::with_capacity(count_tlvs_t8_l16(segment_id_buf));
    ...
    fn parse(cur: &mut SliceReader<'a>) -> Result<Self, Self::Error> {
        ...
        let attribute = match tlv_type {
            ...
            BgpSidAttributeType::SRv6ServiceL3 => {
                ...
                let mut subtlvs = Vec::with_capacity(count_tlvs_t8_l16(data));
                while !data.is_empty() {
    ...
    fn parse(cur: &mut SliceReader<'a>) -> Result<Self, Self::Error> {
        ...
        let subtlv = match tlv_type {
            BgpSrv6ServiceSubTlvType::SRv6SIDInformation => {
                ...
                let mut subsubtlvs = Vec::with_capacity(count_tlvs_t8_l16(data));
