...
pub enum PathAttributeParsingError {
    ...
    #[error("in IPv6 extended communities: {0}")]
    ...
    #[error("malformed path attribute {value:?} at byte offset {offset}: {invalid_attribute}")]
...
pub enum OriginParsingError {
    ...
    Parse(#[from] ParseError),
    #[error(
        "invalid ORIGIN attribute length {length} at byte offset {offset} (expected {ORIGIN_LEN})"
    )]
    InvalidOriginLength {
        offset: usize,
    ...
    #[error(
        "unknown ORIGIN value {code} at byte offset {offset} (expected 0=IGP, 1=EGP, 2=INCOMPLETE)"
    )]
...
pub enum AsPathParsingError {
    ...
    #[error("AS_PATH segment has a length of zero at byte offset {offset}")]
    ZeroSegmentLength { offset: usize },
    /// Invalid Length
    #[error(
        "invalid AS_PATH length at byte offset {offset}: found {found} bytes, expected {expecting}"
    )]
    InvalidAsPathLength {
        offset: usize,
        expecting: usize,
    ...
    #[error(
        "unknown AS_PATH segment type {code} at byte offset {offset} (expected 1=AS_SET, 2=AS_SEQUENCE, 3=AS_CONFED_SEQUENCE, 4=AS_CONFED_SET)"
    )]
...
pub enum NextHopParsingError {
    ...
    Parse(#[from] ParseError),
    #[error(
        "invalid NEXT_HOP attribute length {length} at byte offset {offset} (expected {NEXT_HOP_LEN})"
    )]
...
pub enum MultiExitDiscriminatorParsingError {
    ...
    Parse(#[from] ParseError),
    #[error(
        "invalid MULTI_EXIT_DISC attribute length {length} at byte offset {offset} (expected {MULTI_EXIT_DISCRIMINATOR_LEN})"
    )]
    InvalidLength {
        offset: usize,
        length: PathAttributeLength,
...
pub enum LocalPreferenceParsingError {
    ...
    Parse(#[from] ParseError),
    #[error(
        "invalid LOCAL_PREF attribute length {length} at byte offset {offset} (expected {LOCAL_PREFERENCE_LEN})"
    )]
    InvalidLength {
        offset: usize,
        length: PathAttributeLength,
...
pub enum AtomicAggregateParsingError {
    ...
    Parse(#[from] ParseError),
    #[error(
        "invalid ATOMIC_AGGREGATE attribute length {length} at byte offset {offset} (expected {ATOMIC_AGGREGATE_LEN})"
    )]
    InvalidLength {
        offset: usize,
        length: PathAttributeLength,
...
pub enum AggregatorParsingError {
    ...
    Parse(#[from] ParseError),
    #[error(
        "invalid AGGREGATOR attribute length {length} at byte offset {offset} (expected {AS2_AGGREGATOR_LEN} with 2-octet ASNs or {AS4_AGGREGATOR_LEN} with 4-octet ASNs)"
    )]
    InvalidLength {
        offset: usize,
        length: PathAttributeLength,
...
pub enum MpReachParsingError {
    #[error("{0}")]
    Parse(#[from] ParseError),
    #[error("unknown address family {afi} in MP_REACH_NLRI at byte offset {offset}")]
    UndefinedAddressFamily { offset: usize, afi: u16 },
    #[error("unknown subsequent address family {safi} in MP_REACH_NLRI at byte offset {offset}")]
    UndefinedSubsequentAddressFamily { offset: usize, safi: u8 },
    #[error("invalid MP_REACH_NLRI next hop for {address_type}: {error}")]
    IpAddrError {
        address_type: AddressType,
        error: IpAddrParsingError,
    },
    #[error("invalid MP_REACH_NLRI labeled next hop for {address_type}: {error}")]
    LabeledNextHopError {
        address_type: AddressType,
...
/// Counts NLRI elements framed as: an optional 4-byte ADD-PATH path
/// identifier, followed by a 1-byte length *in bits*, followed by
/// `ceil(bits/8)` bytes of payload — the shared wire shape of every
/// prefix-style NLRI this crate parses. `max_bytes` caps the payload skip
/// the same way the real parser does: `Ipv4Net`/`Ipv6Net::parse` never
/// reads past the address family's own width (4/16 bytes) regardless of
/// what the length byte claims, while the MPLS-labeled, MPLS-VPN, and
/// Route Target membership variants have no such cap — their length byte
/// already covers a label stack/route distinguisher/prefix combined, so
/// pass `u8::MAX` (a no-op with `.min()`, since `ceil(u8::MAX/8)` is 32
/// regardless). Whatever internal structure lives inside that
/// already-known-size window never needs to be understood here — the
/// outer length byte alone determines how many bytes to skip, so this
/// never allocates and never touches the real per-element parser.
///
/// Purely advisory: a truncated or malformed buffer just stops the count
/// early rather than returning an error, so it never changes what error
/// the real parsing loop reports — only the capacity hint.
#[inline]
fn count_prefix_style_elements(mut cur: SliceReader<'_>, add_path: bool, max_bytes: u8) -> usize {
    let mut count = 0usize;
    loop {
        if cur.is_empty() {
            return count;
        }
        if add_path && cur.take_slice(4).is_err() {
            return count;
        }
        let Ok(bit_len) = cur.read_u8() else {
            return count;
        };
        let byte_len = bit_len.div_ceil(8).min(max_bytes);
        if cur.take_slice(byte_len as usize).is_err() {
            return count;
        }
        count += 1;
    }
...
/// Counts EVPN NLRI elements: an optional 4-byte ADD-PATH path identifier
/// followed by RFC 7432's route framing (1-byte route type, 1-byte
/// length, `length` bytes of route-type-specific payload — the same
/// outer shape regardless of which route type it is).
///
/// Purely advisory, same contract as [`count_prefix_style_elements`].
#[inline]
fn count_l2evpn_elements(mut cur: SliceReader<'_>, add_path: bool) -> usize {
    let mut count = 0usize;
    loop {
        if cur.is_empty() {
            return count;
        ...
        let Ok(_route_type) = cur.read_u8() else {
            return count;
        };
    ...
    fn parse(
        ...
        match AddressType::from_afi_safi(afi, safi) {
            Ok(addr_type @ AddressType::Ipv4Unicast) => {
                ...
                let mut nlri = Vec::with_capacity(count_prefix_style_elements(mp_buf, add_path, 4));
            ...
            Ok(addr_type @ AddressType::Ipv4NlriMplsLabels) => {
                ...
                let add_path = add_path_map.get(&addr_type).is_some_and(|x| *x);
                let mut nlri =
                    Vec::with_capacity(count_prefix_style_elements(mp_buf, add_path, u8::MAX));
                while !mp_buf.is_empty() {
            ...
            Ok(addr_type @ AddressType::Ipv6Unicast) => {
                ...
                let add_path = add_path_map.get(&addr_type).is_some_and(|x| *x);
                let mut nlri =
                    Vec::with_capacity(count_prefix_style_elements(mp_buf, add_path, 16));
                while !mp_buf.is_empty() {
            ...
            Ok(addr_type @ AddressType::L2VpnBgpEvpn) => {
                ...
                let _ = mp_buf.read_u8()?;
                let add_path = add_path_map.get(&addr_type).is_some_and(|x| *x);
                let mut nlri = Vec::with_capacity(count_l2evpn_elements(mp_buf, add_path));
                while !mp_buf.is_empty() {
...
pub enum MpUnreachParsingError {
    ...
    #[error("unknown address family {afi} in MP_UNREACH_NLRI at byte offset {offset}")]
    UndefinedAddressFamily { offset: usize, afi: u16 },

    #[error("unknown subsequent address family {safi} in MP_UNREACH_NLRI at byte offset {offset}")]
    UndefinedSubsequentAddressFamily { offset: usize, safi: u8 },
...
pub enum ClusterIdParsingError {
    #[error("while parsing cluster id: {0}")]
    Parse(#[from] ParseError),
}
...
impl<'a> ParseFromWithOneInput<'a, bool> for ClusterList {
    ...
    fn parse(cur: &mut SliceReader<'a>, extended_length: bool) -> Result<Self, Self::Error> {
        ...
        let mut cluster_ids = Vec::with_capacity(data_buf.remaining() / 4);
...
pub enum AigpParsingError {
    ...
    #[error("unknown AIGP TLV type {aigp_attribute_type} at byte offset {offset}")]
    UndefinedAigpAttributeType {
        offset: usize,
        aigp_attribute_type: u8,
    },

    #[error("invalid AIGP TLV length {length} at byte offset {offset}")]
