...
#[rstest]
fn test_pmacct_bgp(#[files("../../assets/pcaps/pmacct-tests/*/*.pcap")] path: PathBuf) {
    let overwrite = std::env::var("OVERWRITE")
        .to_owned()
        .unwrap_or_default()
        .eq_ignore_ascii_case("true");
    test_bgp_pcap(overwrite, path);
}

#[rstest]
fn test_pcap_bgp(#[files("../../assets/pcaps/bgp/*/*.pcap")] path: PathBuf) {
    let overwrite = std::env::var("OVERWRITE")
        .to_owned()
        .unwrap_or_default()
        .eq_ignore_ascii_case("true");
...
// Rust is not smart enough to detect this method is used in [for_each_file]
#[allow(dead_code)]
fn test_bgp_pcap(overwrite: bool, pcap_path: PathBuf) {
    ...
    for (src_ip, src_port, dst_ip, dst_port, protocol, value) in iter {
        if protocol != TransportProtocol::TCP || dst_port != 179 {
            continue;
        }
        ...
        while buf.has_remaining() {
            let serialized = match codec.decode(buf) {
                ...
                Err(err) => serde_json::to_string(&err)
                    .expect("Couldn't serialize BGP error message to json"),
            };
