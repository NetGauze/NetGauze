...
fn test_mp_reach_nlri_ipv6() -> Result<(), MpReachWritingError> {
    ...
    let unknown_address_type = MpReach::Unknown {
        ...
        value: unknown_address_type_wire[4..].into(),
    };
...
fn test_mp_reach_nlri_ipv4_mpls_labels_ipv6_next_hop() -> Result<(), MpReachWritingError> {
    ...
    let good_no_link_local = MpReach::Ipv4NlriMplsLabels {
        next_hop: IpAddr::V6(Ipv6Addr::new(0x2001, 0x0db8, 0x91, 0, 0, 0, 0, 0x1)),
        next_hop_local: None,
        nlri: Box::new([Ipv4NlriMplsLabelsAddress::from(
            None,
...
fn test_parse_path_attribute_mp_reach_nlri_ipv4_multicast() -> Result<(), PathAttributeWritingError>
{
    ...
    let good = PathAttribute::from(
        ...
        PathAttributeValue::MpReach(MpReach::Ipv4Multicast {
            next_hop: IpAddr::V4(Ipv4Addr::new(172, 16, 0, 20)),
            next_hop_local: None,
            nlri: Box::new([Ipv4MulticastAddress::new_no_path_id(
...
/// Unknown BGP Prefix-SID TLVs, sub-TLVs and sub-sub-TLVs must be preserved
/// verbatim rather than rejected, so that a speaker stays interoperable with
/// peers that send types it doesn't implement yet.
#[test]
pub fn test_segment_identifier_unknown_tlvs_are_preserved() {
    let good_wire = [
        46, // Prefix-SID attribute length
        // --- TLV: SRv6ServiceL3 (type 5), length 38 ---
        5, 0x00, 0x26, 0x00, // reserved
        // --- sub-TLV: SRv6SIDInformation (type 1), length 28 ---
        1, 0x00, 0x1c, 0x00, // reserved1
        // SID 2001:db8::
        0x20, 0x01, 0x0d, 0xb8, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
        0x00, 0x00, // service_sid_flags
        0x00, 0x2f, // endpoint_behaviour = 47
        0x00, // reserved2
        // --- sub-sub-TLV: unknown type 99, length 4 (starts at offset 32) ---
        99, 0x00, 0x04, 0xde, 0xad, 0xbe, 0xef,
        // --- sub-TLV: unknown type 77, length 3 (starts at offset 39) ---
        77, 0x00, 0x03, 0x01, 0x02, 0x03,
        // --- TLV: unknown type 200, length 2 (starts at offset 45) ---
        200, 0x00, 0x02, 0xaa, 0xbb,
    ];

    let good = PrefixSegmentIdentifier::new(vec![
        BgpSidAttribute::SRv6ServiceL3 {
            reserved: 0,
            subtlvs: vec![
                SRv6ServiceSubTlv::SRv6SIDInformation {
                    reserved1: 0,
                    sid: Ipv6Addr::from_str("2001:db8::").unwrap(),
                    service_sid_flags: 0,
                    endpoint_behaviour: 47,
                    reserved2: 0,
    ...
    test_parsed_completely_with_one_input_bytes_reader(&good_wire, false, &good);
    test_write_with_one_input(&good, false, &good_wire).expect("Failed to write");
}
