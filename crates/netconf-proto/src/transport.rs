...
use crate::capabilities::{Capability, YangLibrary};
use crate::protocol::{Hello, NetConfMessage, Rpc, RpcReplyValue};
use futures::{SinkExt, StreamExt};

pub struct SshNetConfClient<E: From<std::io::Error> + std::error::Error + Send + Sync+ 'static, T: AsyncRead + AsyncWrite, C: Decoder<Item = NetConfMessage, Error=E> + Encoder<NetConfMessage, Error=E>> {
    framed: Framed<T, C>,
...
impl<E: From<std::io::Error>+ std::error::Error + Send + Sync + 'static, T: AsyncRead + AsyncWrite + Unpin, C: Decoder<Item = NetConfMessage, Error=E> + Encoder<NetConfMessage, Error = E> > SshNetConfClient<E, T, C> {
    pub fn new(framed: Framed<T, C>) -> Self {
        let mut ctx = Context::new(ContextFlags::NO_YANGLIBRARY).expect("Failed to create context");
        ctx.set_searchdir("../../assets/yang/").expect("Failed to set YANG search directory");
        ctx.load_module(
            "ietf-netconf",
            Some("2011-06-01"),
            &[
        ...
        Self { framed, peer_caps: HashMap::new(), session_id: 0, ctx, next_message_id: 1 }
    }
    ...
    pub async fn hello(&mut self) -> anyhow::Result<()> {
        ...
        self.framed.send(NetConfMessage::Hello(Hello {
            session_id: None,
    ...
    pub async fn get_yang_lib(&mut self) -> anyhow::Result<DataTree<'_>> {
        if let Some(Capability::YangLibrary(YangLibrary::V1_1 {revision: _, content_id}))= self.peer_caps.get(&Box::from(":yang-library:1.1")) {
            let content_id = content_id.parse::<u32>()?;
            eprintln!("content id {content_id:#x}");
            let operation = format!("<get><filter type=\"subtree\"><yang-library xmlns=\"urn:ietf:params:xml:ns:yang:ietf-yang-library\"></yang-library></filter></get>").to_string();
            let message_id = format!("{}", self.next_message_id);
            self.next_message_id += 1;
            self.framed.send(NetConfMessage::Rpc(Rpc {
                message_id,
                operation
            }))
                .await?;
