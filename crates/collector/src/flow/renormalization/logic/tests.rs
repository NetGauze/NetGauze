...
// Tests for renormalize_fields() function
#[test]
fn test_no_renormalization() {
    let fields: Box<[Field]> =
        vec![Field::octetDeltaCount(100), Field::packetDeltaCount(10)].into_boxed_slice();
    let expected = fields.clone();
    let result = renormalize_fields(fields, &create_test_ctx(), &create_stats(), &[]);
    assert_eq!(result, expected);
}

#[test]
fn test_selector_algorithm_systematic_count_based() {
    let fields: Box<[Field]> = vec![
...
fn test_sampler_mode_missing_interval() {
    let fields: Box<[Field]> = vec![Field::samplerMode(1)].into_boxed_slice();
    let expected = fields.clone();
...
fn test_sampler_mode_unsupported() {
    let fields: Box<[Field]> =
        vec![Field::samplerMode(99), Field::samplerRandomInterval(10)].into_boxed_slice();
    let expected = fields.clone();
...
fn test_sampling_algorithm_missing_interval() {
    let fields: Box<[Field]> = vec![Field::samplingAlgorithm(1)].into_boxed_slice();
    let expected = fields.clone();
...
fn test_sampling_algorithm_unsupported() {
    let fields: Box<[Field]> =
        vec![Field::samplingAlgorithm(99), Field::samplingInterval(10)].into_boxed_slice();
    let expected = fields.clone();
...
fn test_inferred_sampling_interval() {
    let fields: Box<[Field]> =
        vec![Field::octetDeltaCount(100), Field::samplingInterval(10)].into_boxed_slice();
...
#[test]
fn test_renormalize_ipfix_empty_flow() {
...
#[test]
fn test_renormalize_ipfix_data_set_with_sampling() {
    let export_time = Utc.with_ymd_and_hms(2025, 1, 1, 10, 0, 0).unwrap();
...
#[test]
#[traced_test]
fn test_renormalize_ipfix_filters_templates_and_options() {
    let export_time = Utc.with_ymd_and_hms(2025, 1, 1, 10, 0, 0).unwrap();
...
#[test]
#[traced_test]
fn test_renormalize_netflow_v9_filters_templates_and_options() {
    let export_time = Utc.with_ymd_and_hms(2025, 1, 1, 10, 0, 0).unwrap();
    let flow_info = FlowInfo::NetFlowV9(netflow::NetFlowV9Packet::new(
        3600000,
        export_time,
        1,
        1,
        Box::new([
            ...
            netflow::Set::OptionsTemplate(Box::new([])),
            netflow::Set::Data {
                id: DataSetId::new(256).unwrap(),
                records: Box::new([netflow::DataRecord::new(
                    Box::new([]),
                    Box::new([Field::octetDeltaCount(500)]),
                )]),
            },
    ...
    let expected = FlowInfo::NetFlowV9(netflow::NetFlowV9Packet::new(
        3600000,
        export_time,
        1,
        1,
