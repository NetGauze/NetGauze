...
use netgauze_flow_pkt::{ie, ie::*, FlatFlowInfo};
...
impl AggregationActor {
    ...
    async fn run(mut self) -> anyhow::Result<String> {
        let agg = self
            .rx
            .flat_map(move |req| {
                self.stats.received_messages.add(1, &[]);
                ...
                stream::iter(
                    flow.flatten()
                        ...
                        .filter(|flow| match flow {
                            FlatFlowInfo::IPFIX(packet) => packet.set().id() >= 256,
                            _ => false,
                        })
                    ...
                    match result {
                        Some(Either::Left(((window_start, window_end), cache))) => {
                            for (_key, message) in cache {
                                ...
                                self.stats.aggregated_messages.add(1, &[]);

                                // info!("******************************************");
                                // info!("**********Window: ({}, {})", window_start, window_end);
                                // info!("**********Key: {}", key);
                                // info!("Aggr Result: {}", serde_json::to_string(&message).unwrap());

                                // TODO: handle error here better
                                self.tx.send(((window_start, window_end), (message.peer, message.flow))).await?;
                            }
                        }
                        Some(Either::Right(_)) => {
