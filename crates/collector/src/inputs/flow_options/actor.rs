...
pub struct FlowOptionsActorStats {
    ...
    netflow_v9_conversion_errors: opentelemetry::metrics::Counter<u64>,
}
                                ...
                                FlowInfo::NetFlowV9(pkt) => {
                                    for set in pkt.into_sets() {
                                        ...
                                        for record in data_records
                                            ...
                                            let options_record = match record.try_into() {
                                                Ok(rec) => rec,
                                                Err(e) => {
                                                    self.stats.netflow_v9_conversion_errors.add(1, &peer_tags);
                                                    debug!("Failed to convert NetFlowV9 record to OptionsDataRecord: {e}");
                                                    continue;
                                                }
                                            };
