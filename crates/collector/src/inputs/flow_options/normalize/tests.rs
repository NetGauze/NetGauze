...
fn test_netflow_try_from_cache_scope() {
    ...
    let result = OptionsDataRecord::try_from(record).unwrap();
    let OptionsDataRecord::Unclassified(unclassified_record) = result else {
        panic!("Expected Unclassified type");
    };
    let expected = IndexedDataRecord::new(&[], &[Field::octetDeltaCount(999)]);
    assert_eq!(unclassified_record, expected);
}
...
fn test_netflow_try_from_template_scope() {
    ...
    let expected = IndexedDataRecord::new(&[], &[Field::octetDeltaCount(42)]);
    assert_eq!(unclassified_record, expected);
}
...
fn test_netflow_try_from_unknown_scope() {
    ...
    let expected = IndexedDataRecord::new(&[], &[Field::octetDeltaCount(1)]);
    assert_eq!(unclassified_record, expected);
}
