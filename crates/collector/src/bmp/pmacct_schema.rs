...
/// A newtype around `Vec<PmacctBmpMessage>` that enables a `TryFrom` impl
/// without violating the orphan rule (which forbids
/// `impl TryFrom<…> for Vec<_>` because `Vec` is a foreign type).
///
/// ```rust,ignore
/// let msgs = PmacctBmpMessages::try_from((request, &ctx))?;
/// for msg in msgs.into_inner() { /* … */ }
/// ```
#[derive(Debug, Clone)]
pub struct PmacctBmpMessages(Vec<PmacctBmpMessage>);

impl PmacctBmpMessages {
    pub fn into_inner(self) -> Vec<PmacctBmpMessage> {
        self.0
    }

    pub fn as_slice(&self) -> &[PmacctBmpMessage] {
        &self.0
    }
}

impl TryFrom<(&BmpRequest, &PmacctConversionContext)> for PmacctBmpMessages {
    type Error = PmacctBmpConversionError;

    fn try_from(
        (request, ctx): (&BmpRequest, &PmacctConversionContext),
    ) -> Result<Self, Self::Error> {
        PmacctBmpMessage::try_from_bmp_request(request, ctx).map(PmacctBmpMessages)
    }
}
