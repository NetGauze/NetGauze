...
async fn test_peer_management_disconnect() {
    ...
    let (actor_id, peers) = handle.get_connected_peers().await.unwrap();
    assert!(
    ...
    // Disconnect peer
    let disconnected = handle.disconnect_peer(peer_addr).await.unwrap();
    assert!(disconnected, "Should return true for disconnected peer");

    // Verify peer is gone from list
    let (_, peers) = handle.get_connected_peers().await.unwrap();
    assert!(
        !peers.contains(&peer_addr),
    ...
    let disconnected_again = handle.disconnect_peer(peer_addr).await.unwrap();
    assert!(
        !disconnected_again,
...
async fn test_sharded_subscription() {
    ...
    let (receivers, _) = handle.subscribe_shards(2, 10).await.unwrap();
    assert_eq!(receivers.len(), 2);

    // Connect and send
    let stream = TcpStream::connect(handle.local_addr()).await.unwrap();
    let mut framed = FramedWrite::new(stream, BmpCodec::default());
    framed.send(create_test_message()).await.unwrap();
...
async fn test_subscription_unsubscribe() {
    ...
    // Subscribe
    let (rx, sub) = handle.subscribe(10).await.unwrap();
    ...
    // Unsubscribe
    let result = handle.unsubscribe(sub.id).await.unwrap();
...
async fn test_subscription_updates_existing_connection() {
    ...
    let (rx, _sub) = handle.subscribe(10).await.unwrap();
