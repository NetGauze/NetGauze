...
/// [TaggedData] with the original input from the stream and a copy of the tag.
///
/// Note, the sender which implements [AsyncWrite] is optional, in case we need
/// keep bidirectional connections such as TCP open. By passing the write side
/// of the connection and keeping it, we guarantee that both sides of the
